...
- **矩形** (`Ctrl+Shift+E` / ポップアップ) ドラッグで囲んだところに適用する。ウィンドウ基準の固定座標なので、
...
ポップアップとショートカット `Ctrl+Shift+E` / `Ctrl+Shift+L` は、いま選んでいるモードで適用する。
右クリックの「隠す」と `Ctrl+Shift+Y` は赤枠を選んでいても隠す側になり、最後に選んだ
モザイク・ぼかし・黒塗りのどれかを使う。
